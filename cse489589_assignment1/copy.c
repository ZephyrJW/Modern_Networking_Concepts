 ...
 void processCmd(char **cmd, int count){
     if (strcmp(cmd[0], "AUTHOR") == 0) {
		...
		// mark
         ...
         send(clientsockfd, "-~!~-REFRESH-~!~-", 17, 0);
         char update[BUFLEN];
         ...
         strcat(buf, "-~!~-");
         strcat(buf, msg);
		 //debug used to be sizeof buf
 	     send(clientsockfd, buf, BUFLEN, 0);// sizeof buf  also works?
